...
window.addEventListener("click", (e) => {
  ...
  if (e.target.closest("[data-evnt-hdr-btn-elmnt]")) {
    ...
    if (document.documentElement.clientWidth > 900 && !document.querySelector("[data-number-nav].active") && parent.classList.contains("active")) {
      toggleStatus.call(null, { mode: "close", selector: "[data-number-nav]" });
      toggleStatus.call(firstLi, {});
      toggleSubmodalMenu.call(firstLi, {});
    }
  } else !e.target.closest("[data-evnt-hdr-btn-mdl]") && toggleStatus({ mode: "close", selector: "[data-evnt-hdr-btn]" });

  // navigation submenu
  if (e.target.closest("[data-number-nav]")) {
    const _this = e.target.closest("[data-number-nav]");
    toggleStatus.call(_this, {});
    toggleStatus.call(_this, { mode: "close", selector: "[data-number-nav]" });
  ...
  if (e.target.closest("[data-evnt-brgr]")) {
    ...
    document.documentElement.clientWidth <= 500 ? toggleModal({ modal: nav }) : toggleStatus.call(nav, {});

    toggleStatus.call(e.target.closest("[data-evnt-brgr]"), { modificator: "open" });
  } else {
    if (!e.target.closest("[data-evnt-hdr-nv]") && document.documentElement.clientWidth > 500) {
      toggleStatus.call(null, { mode: "close", modificator: "open", selector: "[data-evnt-brgr]" });
      toggleStatus.call(null, { mode: "close", selector: "[data-evnt-hdr-nv]" });
    }
  }
  ...
  if (e.target.closest("[data-evnt-slct-bttn]")) {
    ...
    toggleStatus.call(null, { mode: "close", selector: "[data-select-input]" });
  }
  ...
  if (e.target.closest("[data-select-input-element]")) {
    ...
    toggleStatus({ mode: "close", selector: selectorElement });
  ...
  if (e.target.closest("[data-event-fltr-bttn]")) {
    e.preventDefault();
    toggleModal({ modal: document.querySelector("[data-modal-tlbr]") });
  ...
  if (e.target.closest("[data-event-drpdwn-slct-bttn]")) {
    ...
    if (!e.target.closest("[data-event-drpdwn-slct-mdl]")) toggleStatus({ mode: "close", selector: "[data-event-drpdwn-slct]" });
  }
  // pick
  if (e.target.closest("[data-event-drpdwn-slct-elmnt]")) {
    ...
    toggleStatus({ mode: "close", selector: selector, parent: parent });
    toggleStatus.call(element, {});
    toggleStatus({ mode: "close", selector: parentSelector });
